class MovieModel {
  ...
  final String poster;
  final String id;
  final String backdrop, overview;
  final double voteAverage, popularity;
...
class MovieModelList {
  ...
  MovieModelList({required this.movies});
  factory MovieModelList.fromJson(List<dynamic> json) {
    return MovieModelList(movies: (json).map((list) => MovieModel.fromJson(list)).toList());
  }
}
...
class MovieInfoModel {
  ...
  factory MovieInfoModel.fromJson(json) {
    return MovieInfoModel(
      ...
      imdbid: json['imdb_id'] ?? "",
      languages: (json['spoken_languages'] as List).map((laung) => laung['english_name']).toList(),
...
class TrailerList {
  ...
  factory TrailerList.fromJson(json) {
    return TrailerList(trailers: (json['results'] as List).map((trailer) => TrailerModel.fromJson(trailer)).toList());
  }
}
...
class ImageBackdropList {
  ...
  factory ImageBackdropList.fromJson(backdrops) {
    return ImageBackdropList(
      backdrops: (backdrops as List).map((backdrop) => ImageBackdrop.fromJson(backdrop)).toList(),
    );
  }
}
...
class CastInfo {
  ...
  factory CastInfo.fromJson(json) {
    return CastInfo(
      ...
      image: json['profile_path'] != null ? "https://image.tmdb.org/t/p/w500" + json['profile_path'] : "",
    );
  }
}
...
class CastInfoList {
  ...
  factory CastInfoList.fromJson(json) {
    return CastInfoList(
      castList: (json['cast'] as List).map((cast) => CastInfo.fromJson(cast)).toList(),
    );
  }
}
